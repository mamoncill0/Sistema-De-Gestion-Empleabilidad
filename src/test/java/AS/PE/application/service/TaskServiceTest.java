...
class TaskServiceTest {
    ...
    void completeTask_TaskNotFound_ShouldFail() {
        ...
        // Act & Assert
        assertThrows(ResourceNotFoundException.class, () -> taskService.completeTask(taskId, ownerId, Role.ADMIN));
    }

    @Test
    @DisplayName("CompleteTask_UserNotOwner_ShouldFail")
    void completeTask_UserNotOwner_ShouldFail() {
        // Arrange
        when(taskRepositoryPort.findById(taskId)).thenReturn(Optional.of(testTask));
        when(projectRepositoryPort.findById(projectId)).thenReturn(Optional.of(testProject));

        // Act & Assert
        assertThrows(UnauthorizedOperationException.class, () -> taskService.completeTask(taskId, otherUserId, Role.USER));
    }
}
